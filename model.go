...
func (Listingmodel ListingModel) GetListingsList(Input ListingInput, DB *gorm.DB) (listing []TblListing, err error) {
	baseQuery := DB.Debug().Table("tbl_listings").
		...
		Where(" tbl_listings.tenant_id = ? AND tbl_listings.is_deleted = 0 and ce1.is_deleted=0 and ce1.status=1 and tbl_listings.status=1", Input.TenantId)
	...
	if len(validIDs) > 0 {
		baseQuery = baseQuery.Where("tbl_listings.id IN (?) AND tbl_listings.tenant_id = ?", validIDs, Input.TenantId)
	}
	...
	// Updated category_id condition - now checks tbl_channel_entries.category_id (string containing comma-separated values)
